...
### 한 화면에 여러 장이 서 있어도 그냥 된다

PC 의 스트림 모드는 한 화면에 프레임을 1·2·4장까지 나란히 세울 수 있다. 카메라 프레임이
16:9 인데 QR 은 정사각형이라, 한 장만 비추면 좌우가 통째로 노는 것을 쓰는 것이다.

**이 앱은 그것을 위해 고친 곳이 없다.** 두 자리가 이미 그렇게 되어 있었다:

- `www/bridge.js` 의 `onBarcodes` 는 `event.barcodes` **배열**을 훑는다. ML Kit 은 한 카메라
  프레임에서 찾은 심볼을 전부 넘겨 주고, 이벤트 이름이 복수형 `barcodesScanned` 인 것이 그
  뜻이다.
- 파운틴 부호라 프레임에 순서가 없다. 번호는 헤더에 들어 있고, `stream.js` 의 `seen` 이 중복을
  가려 낸다 — 한꺼번에 들어온 넷이든 하나씩 들어온 넷이든 결과가 같다.

그래서 이쪽에서 지켜야 하는 계약은 하나뿐이다: **한 번에 받은 심볼을 하나도 버리지 말 것.**
`handleBarcode` 를 심볼 하나마다 부르는 지금 모양을 유지하면 된다.
